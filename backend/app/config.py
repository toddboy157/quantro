...
# Hard per-symbol ceiling (seconds) on a single get_chain() call, including
# every page a paginated vendor response takes. Found live: a large index
# chain (SPX) can apparently paginate for minutes without ever raising an
# error, which - since this is a sequential for-loop over UNDERLYINGS -
# blocks every symbol after it from ever refreshing, with nothing showing
# up in _errors because no individual request technically "failed." This
# bounds that: past this many seconds the fetch is treated as failed (a
# clear, visible error) so the loop can move on to the next symbol instead
# of stalling indefinitely. 25s comfortably covers a normal-sized chain even
# on a slow connection while still failing well within a human noticing.
SYMBOL_FETCH_TIMEOUT_SECONDS = float(os.getenv("SYMBOL_FETCH_TIMEOUT_SECONDS", "25"))
